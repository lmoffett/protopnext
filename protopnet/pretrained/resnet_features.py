...
from .google_drive import ensure_pretrained_model_from_google_drive

model_urls = {
    "resnet18": "https://download.pytorch.org/models/resnet18-5c106cde.pth",
    "resnet34": "https://download.pytorch.org/models/resnet34-333f7ec4.pth",
    "resnet50": "https://download.pytorch.org/models/resnet50-0676ba61.pth",
    "resnet50[pretraining=inaturalist]": "1yY9LcLN7bVy7CmH77Mc4TfmW7qT00oS9",
...
def resnet50_features(pretrained=False, pretraining="imagenet1k", **kwargs):
    ...
    if pretrained:
        if pretraining == "inaturalist":
            model_dir_path = pathlib.Path(model_dir)
            model_path = (
                model_dir_path / "BBN.iNaturalist2017.res50.90epoch.best_model.pth"
            )
            ensure_pretrained_model_from_google_drive(
                model_urls[f"resnet50[pretraining={pretraining}]"], model_path
            )
            my_dict = torch.load(
        ...
        elif pretraining == "imagenet1k":
            my_dict = model_zoo.load_url(model_urls["resnet50"], model_dir=model_dir)
            my_dict.pop("fc.weight")
            my_dict.pop("fc.bias")
            model.load_state_dict(my_dict, strict=False)
        else:
            raise ValueError("pretraining must be either 'imagenet1k' or 'inaturalist'")
