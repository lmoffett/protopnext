...
from .embedding import AddonLayers, LightweightAddonLayers
from .losses import ClassAwareExtraCalculations
from .model_losses import loss_for_coefficients
from .models.st_protopnet import STProtoPNet, STProtoPNetTrainingSchedule
...
@click.command("train-support-trivial")
@common_training_options
@click.option(
    "--proto-channels",
    default=64,
    type=int,
    help="Number of channels in the prototype layer (override)",
)
@click.option(
    "--num-prototypes-per-class",
    default=10,
    type=int,
...
def run(
    *,
    ...
    cluster_coef=-0.8,
    support_separation_coef=0.48,
    trivial_separation_coef=0.08,
    l1_coef=0.0001,
    addon_layer_type="lightweight",
    num_addon_layers=1,
    ...
    rsample_init=False,
    orthogonality_loss_coef=0.001,
    p=1,
    closeness_loss_coef=1.0,  # FIXME: there should be negative here but our loss function has it built in
    discrimination_loss_coef=1.0,
    run_id=None,
    weight_decay_multiplier=1.0,
):
    """
    Train a ST ProtoPnet (currently only supports Support or Trivial models).
    ...
    - joint_epochs_per_phase: int -
    - last_only_epochs_per_phase: int - coefficient for clustering loss
    - cluster_coef: float - coefficient for clustering loss term
    - support_separation_coef: float - coefficient for separation loss term on support vectors
    - trivial_separation_coef: float - coefficient for separation loss term on trivial vectors
    - l1_coef: float - coefficient for clustering loss
    - addon_layer_type: str -
    - num_addon_layers: int -
    ...
    - orthogonality_loss_coef: float -
    - p: int -
    - closeness_loss_coef: float -
    - discrimination_loss_coef: float -
    - run_id: int -
    ...
    log.log(
        ...
        f'Running st-protopnet training as a {"dry_run" if dry_run else "full run"}',
    )

    setup = {
        "support_num_prototypes_per_class": num_prototypes_per_class // 2,
        "trivial_num_prototypes_per_class": num_prototypes_per_class // 2,
        "support_coefs": {
    ...
    if fa_coef is not None and fa_coef != 0:
    ...
    # NOTE: we set base LR here before backbone variable switches str --> torch.nn
    upsample_add_on_layer = bool("vgg" not in backbone)

    if addon_layer_type == "lightweight":

        # NOTE: ST-ProtoPNet upsamples backbones (this codebase usually down samples)
        if upsample_add_on_layer:
            backbone = construct_backbone(backbone)
        else:
            # this is the case for VGG19
        ...
        add_on_layers = LightweightAddonLayers(
            num_prototypes=setup["support_num_prototypes_per_class"]
    ...
    support_vppn = VanillaProtoPNet(
        ...
        num_prototypes_per_class=setup["support_num_prototypes_per_class"],
    )
    ...
    trivial_vppn = VanillaProtoPNet(
        ...
        activation=activation,
        num_classes=split_dataloaders.num_classes,
        num_prototypes_per_class=setup["trivial_num_prototypes_per_class"],
    )

    # join models
    # FIXME: use st protopnet, make join super class
    support_trivial_vppn = STProtoPNet(models=[support_vppn, trivial_vppn])
    ...
    training_schedule = STProtoPNetTrainingSchedule(
        ...
        weight_decay=1e-4 * weight_decay_multiplier,
